...
* O aluno deverá fazer duas avaliações (nota de 0 a 100) e estará aprovado se alcançar uma nota maior ou igual a 70 e se 
* tiver frequência de no mínimo 75%. Caso o aluno não alcance a média 70 mas tenha frequência suficiente então ele tem direito 
* a realizar o EXAME. Neste caso, para ele ser aprovado, a média ponderada entre o primeiro resultado (com peso de 60%) e a nota do 
* exame (com peso de 40%) deverá ser maior ou igual a 50.
    */
//iniciar variaveis
float nota1 = 0f;
float nota2 = 0f;
...
//calcular média
    media = (nota1+nota2)/2;
//calcular frequencia baseado no numero de aulas-faltas
...
//testar media e frequencia
        if (media >=70 && total_frequencia >=75){
                ...
                JOptionPane.showMessageDialog(null,"O aluno "+nome+" não passou na cadeira de Programação sua nota foi "+media+". E sua frequência foi de "+total_frequencia+".");
                    contador1 = 1;
                    //se media não foi boa, testa frequencia para ver se tem direito a exame
                    if (total_frequencia >=75){
                        contador1 = 1;
                        // define pesos das notas 1 e 2
